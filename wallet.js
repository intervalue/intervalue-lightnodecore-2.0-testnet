...
function readTransactionHistory(opts, handleHistory) {
	...
	var sql = "SELECT unit, level, is_stable, sequence, address, \n\
...
async function determineIfFixedDenominationsForJoint(asset, handleResult) {
	if (!asset)
		return await handleResult(false);
	await storage.readAsset(db, asset, null, function (err, objAsset) {
		if (err)
			throw Error(err);
...
async function readAdditionalSigningAddressesForJoint(arrPayingAddresses, arrSigningAddresses, arrSigningDeviceAddresses, handleAdditionalSigningAddresses) {
	...
	await db.query(
		...
		async function (rows) {
			...
			await readAdditionalSigningAddressesForJoint([], arrSigningAddresses.concat(arrAdditionalAddresses), arrSigningDeviceAddresses, async function (arrMoreAddresses) {
				await handleAdditionalSigningAddresses(arrAdditionalAddresses.concat(arrMoreAddresses));
			});
		}
	);
}
...
async function readFundedAndSigningAddressesForJoint(
	...
	await readFundedAddressesForJoint(asset, wallet, estimated_amount, spendUnconfirmed, async function (arrFundedAddresses) {
		...
		var addSigningAddressesAndReturn = async function () {
			var arrPayingAddresses = _.union(arrFundedAddresses, arrBaseFundedAddresses);
			await readAdditionalSigningAddresses(arrPayingAddresses, arrSigningAddresses, arrSigningDeviceAddresses, async function (arrAdditionalAddresses) {
				await handleFundedAndSigningAddresses(arrFundedAddresses, arrBaseFundedAddresses, arrSigningAddresses.concat(arrAdditionalAddresses));
			});
		};
		if (!asset)
		...
		await readFundedAddressesForJoint(null, wallet, TYPICAL_FEE, spendUnconfirmed, async function (_arrBaseFundedAddresses) {
			...
			await readFundedAddressesForJoint(null, fee_paying_wallet, TYPICAL_FEE, async function (_arrBaseFundedAddresses) {
				arrBaseFundedAddresses = _arrBaseFundedAddresses;
				await addSigningAddressesAndReturn();
			});
		});
	});
}
...
function sendMultiPayment1(opts, handleResult) {
	...
	await readFundedAndSigningAddressesForJoint(
		...
		async function (arrFundedAddresses, arrBaseFundedAddresses, arrAllSigningAddresses) {
			...
			if (asset) {
				...
				await composer.composeAndSaveMinimalJointForJoint(params);
			}
...
function replaceInTextcoinTemplate(params, handleText) {
	...
	fs.readFile(__dirname + '/email_template.html', 'utf8', function (err, template) {
		...
		var text = "Here is your link to receive " + params.amount + " " + params.asset + params.usd_amount_str + ": https://inve.one/openapp.html#textcoin?" + params.mnemonic;
...
exports.readAdditionalSigningAddressesForJoint = readAdditionalSigningAddressesForJoint;
exports.determineIfFixedDenominationsForJoint = determineIfFixedDenominationsForJoint;
...
exports.readFundedAndSigningAddressesForJoint = readFundedAndSigningAddressesForJoint;