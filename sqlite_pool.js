...
module.exports = function (db_name, MAX_CONNECTIONS, bReadOnly) {
	...
	async function takeConnectionFromPoolSync() {

		if (!bReady) {
			console.log("takeConnectionFromPool will wait for ready");
			await eventEmitter.once('ready', async function () {
				console.log("db is now ready");
				await takeConnectionFromPoolSync();
			});
			return;
		}
		...
		for (var i = 0; i < arrConnectionSyncs.length; i++)
			if (!arrConnectionSyncs[i].bInUse) {
				...
				return arrConnectionSyncs[i];
			}

		let connection = await connectSync();
		arrConnectionSyncs.push(connection);
		return connection;
	}
	...
	// takes a connection from the pool, executes the single query on this connection, and immediately releases the connection
	async function toList(sql, ...args) {
		//console.log(arguments[0]);
	...
	async function execute(sql, ...args) {
		...
		let result = await connection.query(sql, args);
		connection.release();
		return result;
	}

	function addCmd(cmds, sql, ...args) {
		cmds.push({ sql, args });
	}
	...
	async function executeBatch(cmds) {
		//console.log(arguments[0]);
		var connection = await takeConnectionFromPoolSync();
		try {
	...
	async function executeTrans(cmds) {
		//console.log(arguments[0]);
		var connection = await takeConnectionFromPoolSync();
		try {
	...
	async function single(sql, ...args) {
		//console.log(arguments[0]);
		var connection = await takeConnectionFromPoolSync();
	...
	async function singleList(sql, ...args) {
		...
		if (rows == null || rows.length == 0) {
			...
			return rows.select(p => { return p[proName]; });
		}
	}
	...
	function closeSync(cb) {
		if (!cb)
			cb = function () { };
		bReady = false;
		if (arrConnectionSyncs.length === 0)
			return cb();
